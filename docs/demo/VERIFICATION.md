...
Re-recorded at commit `e88b360`, after the internal refactor of the CLI, runner and adapter. The runtime results below are unchanged, and the replay evidence files were regenerated so they match the current source.

| Check | Observed result |
| --- | --- |
| `npm test` | 29 passed; two integration tests explicitly skipped in this command |
...
| Single repaired replay | All four declared checks passed; target removed; 18.40 seconds |
| Three-revision rehearsal | All expected outcomes observed; targets removed; 54.98 seconds |
