...
import { assertRestrictedTarget, installRevision, sendCheck } from "./target.js";

type RevisionResult = ReplayEvidence["results"][number];

interface RevisionRun {
  adapter: DocksideAdapter;
  root: string;
  runId: string;
  revision: Revision;
 ...
 */
async function replayRevision({ adapter, root, runId, revision, progress, isCancelled }: RevisionRun): Promise<RevisionResult> {
  ...
  try {
    ...
    await assertRestrictedTarget(containerId);
    await installRevision(containerId, root, revision);
    ...
    const address = await assertRestrictedTarget(containerId);
    for (const check of checks) {
      if (isCancelled()) throw new Error("Replay cancelled");
      try {
        observations.push(evaluate(check, await sendCheck(address, check.body)));
      } catch (error) {
...
export async function replay(root: string, selected: readonly Revision[], progress: (message: string) => void): Promise<ReplayEvidence> {
  ...
  try {
    for (const revision of selected) {
      if (cancelled) break;
      const result = await replayRevision({ adapter, root, runId, revision, progress, isCancelled: () => cancelled });
      results.push(result);
